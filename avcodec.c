...
typedef struct h264_codec_context_s {
	...
	AVFrame *decoder_sw_avframe;
	struct SwsContext *decoder_sws_ctx;
	AVBufferRef *decoder_hw_device_ctx;
	enum AVPixelFormat decoder_hw_pix_fmt;
...
static enum AVPixelFormat hw_decoder_get_format(AVCodecContext *ctx, const enum AVPixelFormat *pix_fmts)
{
	h264_codec_context_t *context = (h264_codec_context_t *)ctx->opaque;
	const enum AVPixelFormat *p;

	for (p = pix_fmts; *p != AV_PIX_FMT_NONE; p++) {
		if (*p == context->decoder_hw_pix_fmt) {
			return *p;
		}
	}
...
static switch_status_t open_encoder(h264_codec_context_t *context, uint32_t width, uint32_t height)
{
	...
	if (!context->encoder) {
		if (context->av_codec_id == AV_CODEC_ID_H264) {
			if (context->codec_settings.video.try_hardware_encoder) {
				context->encoder = avcodec_find_encoder_by_name("h264_nvenc");

				if (!context->encoder) {
					context->encoder = avcodec_find_encoder_by_name("nvenc_h264");
				}
	...
	if (decoding) {
		...
		if (context->av_codec_id == AV_CODEC_ID_H264) {
			...
			if (context->decoder) {
				for (i = 0;; i++) {
					...
					if ((config->methods & AV_CODEC_HW_CONFIG_METHOD_HW_DEVICE_CTX) && config->device_type == AV_HWDEVICE_TYPE_CUDA) {
						context->decoder_hw_pix_fmt = config->pix_fmt;
						context->hw_decoder = 1;
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "CUDA HW DECODER ENABLED\n");
						break;
					}
				}
			}
			...
			if (!context->hw_decoder) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "CUDA HW DECODER NOT PRESENT\n");
			}
		}
		...
		context->decoder_ctx->thread_count = context->hw_decoder ? 1 : profile->decoder_thread_count;
		if (context->hw_decoder) {
			context->decoder_ctx->thread_type = 0;
		}

		if (context->hw_decoder) {
			AVDictionary *opts = NULL;

			if (av_hwdevice_ctx_create(&context->decoder_hw_device_ctx, AV_HWDEVICE_TYPE_CUDA, NULL, NULL, 0) < 0) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Could not create CUDA decoder device, trying software decoder\n");
				context->hw_decoder = 0;
			} else {
				context->decoder_ctx->hw_device_ctx = av_buffer_ref(context->decoder_hw_device_ctx);
				context->decoder_ctx->get_format = hw_decoder_get_format;
				context->decoder_ctx->opaque = context;
				av_dict_set(&opts, "extra_hw_frames", "8", 0);
				av_dict_set(&opts, "threads", "1", 0);

				if (avcodec_open2(context->decoder_ctx, context->decoder, &opts) < 0) {
					av_dict_free(&opts);
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Could not open CUDA decoder, trying software decoder\n");
					avcodec_free_context(&context->decoder_ctx);
					av_buffer_unref(&context->decoder_hw_device_ctx);
					context->decoder = avcodec_find_decoder(context->av_codec_id);
					context->hw_decoder = 0;
					context->decoder_ctx = avcodec_alloc_context3(context->decoder);
		...
		if (!decoder_opened && avcodec_open2(context->decoder_ctx, context->decoder, NULL) < 0) {
			if (context->hw_decoder) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Could not open CUDA decoder, trying software decoder\n");
				avcodec_free_context(&context->decoder_ctx);
				av_buffer_unref(&context->decoder_hw_device_ctx);
				...
				if (!context->decoder) {
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Cannot find software decoder id %d\n", context->av_codec_id);
					goto error;
				}
				...
				if (!decoder_opened && avcodec_open2(context->decoder_ctx, context->decoder, NULL) < 0) {
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Error openning software codec\n");
					goto error;
				}
			} else {
...
static switch_status_t copy_decoded_frame_to_image(h264_codec_context_t *context, AVFrame *picture, switch_frame_t *frame)
{
	AVFrame *src = picture;
	const AVPixFmtDescriptor *desc = av_pix_fmt_desc_get((enum AVPixelFormat)picture->format);
	int width, height, ret;

	if (context->hw_decoder || picture->format == AV_PIX_FMT_CUDA || picture->format == context->decoder_hw_pix_fmt || (desc && (desc->flags & AV_PIX_FMT_FLAG_HWACCEL))) {
		if (!context->decoder_sw_avframe) {
			context->decoder_sw_avframe = av_frame_alloc();
			if (!context->decoder_sw_avframe) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Could not allocate software decoder frame\n");
				return SWITCH_STATUS_FALSE;
			}
		}

		av_frame_unref(context->decoder_sw_avframe);
		context->decoder_sw_avframe->format = AV_PIX_FMT_NV12;
		context->decoder_sw_avframe->width = picture->width;
		context->decoder_sw_avframe->height = picture->height;
		ret = av_hwframe_transfer_data(context->decoder_sw_avframe, picture, 0);
		if (ret < 0) {
			char ebuf[255] = "";
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Could not transfer hardware decoded frame to system memory (error '%s')\n", get_error_text(ret, ebuf, sizeof(ebuf)));
			return SWITCH_STATUS_FALSE;
		}

		src = context->decoder_sw_avframe;
	}
	...
	if (src->format == AV_PIX_FMT_YUV420P || src->format == AV_PIX_FMT_YUVJ420P) {
		switch_I420_copy2(src->data, src->linesize,
		...
		uint8_t *dst_y = context->img->planes[SWITCH_PLANE_Y];
		uint8_t *dst_u = context->img->planes[SWITCH_PLANE_U];
		uint8_t *dst_v = context->img->planes[SWITCH_PLANE_V];

		for (y = 0; y < height; y++) {
			memcpy(dst_y + y * context->img->stride[SWITCH_PLANE_Y], src_y + y * src->linesize[0], width);
		}

		for (y = 0; y < height / 2; y++) {
			int x;
			uint8_t *s = src_uv + y * src->linesize[1];
			uint8_t *du = dst_u + y * context->img->stride[SWITCH_PLANE_U];
			uint8_t *dv = dst_v + y * context->img->stride[SWITCH_PLANE_V];
		...
		context->decoder_sws_ctx = sws_getCachedContext(context->decoder_sws_ctx,
												width, height, src->format,
												width, height, AV_PIX_FMT_YUV420P,
												SWS_FAST_BILINEAR,
												NULL, NULL, NULL);
		if (!context->decoder_sws_ctx) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Could not create decoder swscale context\n");
			return SWITCH_STATUS_FALSE;
		}

		sws_scale(context->decoder_sws_ctx,
...
GCC_DIAG_ON(deprecated-declarations)
			...
			if (ret >= 0) {
#endif
				if (copy_decoded_frame_to_image(context, picture, frame) != SWITCH_STATUS_SUCCESS) {
...
static switch_status_t switch_h264_destroy(switch_codec_t *codec)
{
	...
	if (context->decoder_sw_avframe) {
		av_frame_free(&context->decoder_sw_avframe);
	}

	if (context->decoder_sws_ctx) {
		sws_freeContext(context->decoder_sws_ctx);
	}

	if (context->decoder_hw_device_ctx) {
		av_buffer_unref(&context->decoder_hw_device_ctx);
	}
